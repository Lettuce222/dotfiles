# PR review detailed guide

## Contents

- Review labels, tone, and comment length
- Suggestion block semantics
- PR and diff inspection
- Pending review payload and GitHub API behavior
- Editing a pending review
- Submission approval and event types
- Examples and anti-patterns

Use `rg -n '^## |^### ' references/review-guide.md` to locate the relevant section before reading it.
...
3. **pending review として一括投稿** — コメント候補を提示し、作成の承認後に投稿する
...
単発の即時投稿はせず、**コメント案の承認 → pending review に複数コメントを束ねて投稿 → 本人が GitHub UI で確認 → event 種別を承認して submit** という段階的なフローに統一する。

pending review は GitHub 上で **本人にしか見えない** (Files changed タブの自分用下書きレビュー) が、作成・削除は GitHub への書き込みである。会話内では path、line、ラベル、本文要旨を提示して作成承認を取り、投稿後は GitHub UI で suggestion の diff 表示、行ターゲット、バッジの描画を確認してもらう。
...
- **地の文は日本語で書き、英単語を無闇に挟まない**: 英語のまま書いてよいのは、コード識別子・API 名・型名・固有名詞（`TOKEN_TO_TEAM_TTL`、`after_action`、GitHub、N+1 など。識別子はバッククォートで囲む）と、日本語に定訳がなく英語表記が慣用の語だけ。文の述語や修飾語を英単語にしない（「miss します」「fail-open します」「bound されます」「intermittent な」のような英語 + します/されます/な は書かない）。判定基準は「その語でコードやドキュメントを検索するか」— 検索に使う識別子は原文のまま、文章のつなぎや説明は日本語に言い換える。英単語混じりの文は読み手に頭の中の翻訳を強いるうえ、AI 生成特有の文体として悪目立ちする
- **shortcode emoji と全角「！」「？」を適度に混ぜる**。ただし使う emoji は次の 4 種に限定し、Unicode 絵文字（👀 など）の直接埋め込みもしない。温度感の伝達はバッジが担っているので、emoji の種類を増やしても情報は増えずノイズになる:
  - `:pray:` (依頼・お願い)
  - `:bow:` (恐縮・低姿勢・感謝)
  - `:clap:` (称賛)
  - `:+1:` (賛意・後押し)
...
悪い例（述語・修飾語まで英単語になっている）:

> `TOKEN_TO_TEAM_TTL` (1h) > `TEAM_PLAN_TTL` (10min) の非対称で、idle 10〜60 分の窓に来たリクエストだけ plan cache が miss して plan-tier throttle が fail-open します 👀 影響は idle 明け 1 発だけに bound されますが、intermittent bypass の原因になりえます。

良い例（識別子は原文のまま、説明は日本語）:

> `TOKEN_TO_TEAM_TTL` (1 時間) と `TEAM_PLAN_TTL` (10 分) がずれているので、アイドル 10〜60 分の間に来たリクエストだけプランのキャッシュを取り逃して、プラン別のスロットリングが素通りになります :pray: 影響はアイドル明けの 1 リクエストだけに収まりますが、断続的なすり抜けの原因になりえます。

残ってよい英語は `TOKEN_TO_TEAM_TTL` のような識別子だけ。「miss する」→「取り逃す」、「fail-open」→「素通りになる」、「bound される」→「〜に収まる」、「intermittent bypass」→「断続的なすり抜け」のように、説明語は日本語に開く。
...
違い: N+1 の解説 / `includes` の挙動説明 / `:sparkles:`（そもそも許可外の emoji）を落とした。レビュイーは Rails を書いている時点で N+1 と `includes` を知っているので、説明は冗長。suggestion ブロックが差分を示すので「こう書いて」も不要。
...
組み立てたコメント候補の path、line、ラベル、本文をユーザーに提示する。pending review を作成してよいという承認を受けてから、承認済みの候補を一括投稿する。承認前は payload の下書きまでに留め、GitHub API を呼ばない。
...
- 述語・修飾語まで英単語の文（「miss します」「fail-open します」）: 「トーン」セクションの日本語規約と書き直し例に従う
- 許可外の emoji（`:eyes:` `:sparkles:` `:warning:` など）や Unicode 絵文字の直接埋め込み: 使えるのは `:pray:` `:bow:` `:clap:` `:+1:` の 4 種だけ
