---
name: ws
description: 仕事の「全体観」を蓄える個人 Vault（~/workspace の Obsidian markdown）を参照・記録・整理し、タスクを管理するスキル。Christopher Alexander の全体性・パタンランゲージの発想に沿って、仕事の中心・支える中心・弱い中心・次の構造保存的な一手を見つける。PR レビュー時にアーキテクチャ・周辺コード・残タスク・全体ゴールを取り出したい、仕事上の概念や判断の背景を引きたい、イニシアチブの現状を把握したい、知見やタスクを残したい、Vault をお掃除したいとき必ず使う。「全体観」「全体性」「Vault」「ナレッジ」「メモに残して」「これ記録して」「タスク追加して」「今のこの PR の背景」「この概念の意味」「パタン」「pattern」「お掃除」「orient」「unfold」などのキーワードや、ユーザーが特定リポジトリに紐付かない仕事の知識・タスクを出し入れしたい文脈で、明示的に Vault と言われなくても積極的にトリガーすること。特定リポジトリ固有の作業 tips は auto-memory の領分なので、そちらと混同しないこと。
---
...
3. **目的・現状・未解決トピック・関連知識**を簡潔に要約して返す。PR レビューなら「何を達成したくて、今どこで、あと何が残っているか」を含める。
...
- 関係する MOC に新ノートへの `[[リンク]]` を加え、`HOME.md` から到達可能にする。
...
2. スクリプトの出力をもとに、**Claude が判断する**統廃合・整理を提案する: 似た2ノートの統合、古い initiative の `status` 見直し、MOC への未収録ノートの追加、など。
3. 静的検査に加えて wholeness review を行う:
   ...
   - **到達の階層**: HOME → MOC → initiative/concept/pattern → meeting/decision/reference が自然につながるか
