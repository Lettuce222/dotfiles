# ws operations reference

## Contents

- Vault location and directory structure
- Note types and templates
- Orient and capture
- Task CRUD and external sync
- Unfold and cleanup
- Vault boundaries

Use `rg -n '^## |^### ' references/operations.md` to locate one operation before reading it.
...
- **保存対象が明確な依頼は、その範囲の書き込みを承認している。** 書いた後に何をどこへ書いたかを報告する。ただし、retrospective など開始済みの別ワークフローが提案・確認を要求している場合は、その承認境界を優先する。
- 書き先や残す内容が曖昧な場合、またはエージェントが自発的に気づいた場合は、**何を・どのノートに書くかを一度提案し、承認を得てから書く**（誤記録や重複は Vault の信頼性を損なうため）。
- 「まとめて」「整理して」は明示依頼であっても後述の**思考整理モード**として扱い、承認を得てから書く（何をどこに書くかの判断がエージェント側に大きく委ねられるため）。
