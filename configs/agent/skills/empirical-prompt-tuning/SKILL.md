...
description: 自作した agent 向けテキスト指示（skill / slash command / task プロンプト / CLAUDE.md 節 / コード生成プロンプト）を、バイアスを排した実行者 (新規 subagent) に動かしてもらい、両面 (実行者の自己申告 + 指示側メトリクス) で評価して反復改善する手法。改善が頭打ちになるまで回す。業務プロジェクトで作った skill / slash command / CLAUDE.md の節を「新規作成した」「大幅改訂した」とき、または「agent の挙動が期待通りにならない、プロンプトが曖昧なせいかも」「skill の品質を上げたい」「書いたけど本当に伝わってる?」と思ったら必ず使う。
---
...
0. **Iteration 0 — description と body の整合チェック**（静的、dispatch 不要）
   ...
   - 例: description「navigation / form filling / data extraction」と書いてあるが body は CLI reference のみ、のような乖離を検出
