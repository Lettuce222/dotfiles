...
description: ast-grep をプロジェクト lint ツールとして運用するためのガイド。sgconfig.yml 設定、fix/rewrite ルール、constraints、transform、テスト、CI 統合、既存 linter との使い分けを扱う。業務で扱う TypeScript / Ruby / Go コードベースに独自規約を足したい、deprecated API を一括書き換えしたい、ESLint / RuboCop / Biome で表現できない構造的パターン (親子関係・文脈依存) を検出したいとき、必ず使うこと。「この書き方を禁止したい」「移行 migration を安全に回したい」「CLAUDE.md で言葉で縛っているルールを静的チェックに移したい」のような文脈でも発動する。
---
